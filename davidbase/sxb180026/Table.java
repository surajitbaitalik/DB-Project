...
public class Table{
	...
	public static void drop(String table){
		try{
			...
			int number_of_pages = pages(file);
			for(int page = 1; page <= number_of_pages; page ++){
				file.seek((page-1)*pageSize);
				byte fileType = file.readByte();
				if(fileType == 0x0D)
				{
					short[] cellsAddress = BPlusTree.getCellArray(file, page);
					int k = 0;
					for(int i = 0; i < cellsAddress.length; i++)
					{
						long loc = BPlusTree.getCellLoc(file, page, i);
						String[] vals = getResValues(file, loc);
						String tb = vals[1];
						if(!tb.equals(table))
						{
							BPlusTree.setCellOffset(file, page, k, cellsAddress[i]);
							k++;
						}
					}
			...
			File prev_file = new File(dir_userdata, table+".tbl"); 
			prev_file.delete();
		}catch(Exception e){
			System.out.println(e);
		}
	...
	public static String[] getResValues(RandomAccessFile file, long loc){
	...
	public static void createTable(String table, String[] col){
		try{	
			...
			for(int i = 0; i < col.length; i++){
				...
				String[] value = {Integer.toString(l), table, colmnname, dt, pos, nullable, unique};
	...
	public static void update(String table, String[] cmp, String[] set){
		try{
			...
			String[] columns = getColName(table);
			String[] vals = getResValues(file, loc);
			...
			int plsize = calPayloadSize(table, vals, stc);
			BPlusTree.updateLeafCell(file, page, offset, plsize, key, stc, vals);
	...
	public static void insertInto(String table, String[] values,String dir_s) throws Exception{
		try{
	...
	public static void insertInto(RandomAccessFile file, String table, String[] values){
		String[] dttype = getDataType(table);
		String[] nullable = getNullable(table);
		...
		for(int i = 0; i < unique.length; i++)
			if(unique[i].equals("YES")){
				//System.out.println("unique constraint violation check");
				System.out.println();
				...
				getVals(uni, cmp, columnName, type, records);
		...
		byte[] stc = new byte[dttype.length-1];
	...
	public static int calPayloadSize(String table, String[] vals, byte[] len){
		String[] dataType = getDataType(table);
		int size =dataType.length;
		for(int i = 1; i < dataType.length; i++){
			len[i - 1]= getSize(vals[i], dataType[i]);
			size = size + feildLength(len[i - 1]);
		}
		return size;
	}
	

	public static byte getSize(String value, String dataType){
    ...
    public static short feildLength(byte len){
		switch(len){
			case 0x00: 
				return 1;
			case 0x01: 
			...
			case 0x0B: 
				return 8;
			default:   
				return (short)(len - 0x0C);
		}
	}


	
public static int findKeyPg(RandomAccessFile file, int key){
		int val = 1;
		try{
			int npgs = pages(file);
			for(int page = 1; page <= npgs; page++){
				file.seek((page - 1)*pageSize);
	...
	public static String[] getDataType(String table){
		...
		try{
			...
			String[] tab_comp = {"table_name","=",table};
			filter(file, tab_comp, columnName, records);
	...
	public static void display(String table, String[] cols, String[] cmp,String dir_s){
		try{
			String path = dir_userdata ;
			...
			getVals(file, cmp, columnName, type, records);
	...
	public static void getVals(RandomAccessFile file, String[] cmp, String[] columnName, String[] type, Records records){
		try{
			...
			for(int page = 1; page <= numpages; page++){
				...
				byte pageType = file.readByte();
					...
					byte numcells = BPlusTree.getCellNumber(file, page);

					 for(int i=0; i < numcells; i++){
						long offset_loc = BPlusTree.getCellLoc(file, page, i);
						String[] vals = getResValues(file, offset_loc);
						...
						boolean check = checkEqu(vals, rowid , cmp, columnName);
	...
	public static boolean checkEqu(String[] values, int rowid, String[] cmp, String[] columnName){
		...
		if(cmp.length == 0){
			...
			if(columnloc == 1){
				...
				if(cmp[2].equals(values[columnloc-1]))
					check = true;
				else
					check = false;
			}
	...
	public static void index(String table, String[] cols){
		try{
			String path = dir_userdata ;
